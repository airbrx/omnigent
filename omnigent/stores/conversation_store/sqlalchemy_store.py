...
class SqlAlchemyConversationStore(ConversationStore):
    ...
    def usage_totals_for_user(self, user_id: str) -> UsageTotals:
        ...
        with self._session("usage_totals_for_user") as session:
            owned_ids = select(SqlSessionPermission.conversation_id).where(
                SqlSessionPermission.workspace_id == current_workspace_id(),
                SqlSessionPermission.user_id == user_id,
                SqlSessionPermission.level >= LEVEL_OWNER,
            )
            # session_usage moved to the metadata table when conversations was
            # split; join back to conversations to keep only top-level sessions
            # (NULL parent == the "default" kind), excluding sub-agent children.
            rows = (
                session.execute(
                    select(SqlConversationMetadata.session_usage)
                    .join(SqlConversation, SqlConversation.id == SqlConversationMetadata.id)
                    .where(
                        SqlConversationMetadata.workspace_id == current_workspace_id(),
