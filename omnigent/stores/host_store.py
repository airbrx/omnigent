...
def caller_can_reach_host(host: Host, user_id: str | None) -> bool:
    ...
    """
    return user_id is None or host.user_id == user_id or host.visibility == VISIBILITY_SHARED
...
class HostStore:
    ...
    def upsert_on_connect(
        ...
        *,
        ...
        :param version: The host's build label from ``host.hello``
            (``version_label()``), or ``None`` from an older host.
        :param os: The host's OS description from ``host.hello``, or
        ...
        with self._session("upsert_host_on_connect") as session:
            ...
            if row is not None:
                ...
                if os is not None:
                    row.os = os
                # login_token_expires_at refreshes on every connect (it
    ...
    def list_visible_hosts(self, owner: str) -> list[Host]:
        ...
        :returns: List of :class:`Host` entities, ``updated_at`` desc.
        """
        with self._session("list_visible_hosts") as session:
            rows = (
                session.query(SqlHost)
                .filter((SqlHost.user_id == owner) | (SqlHost.visibility == VISIBILITY_SHARED))
                .order_by(SqlHost.updated_at.desc())
                .all()
            )
            return [_row_to_host(row) for row in rows]
