...
export function NewChatLandingScreen() {
  ...
  const renderHostMenuItem = (host: Host) => {
    ...
    // airbrx: an offline host whose cloud compute this server can power on
    // (server `host_wake:` config) becomes a wake affordance rather than a dead
    // row. `wakeable` is false for every host unless an operator configured
    // `host_wake:`, so a laptop-only install never reaches this branch. Kept
    // ahead of the generic disabled path below and after `reconnect`, so this
    // machine still prefers its own connect affordance.
    if (!reconnect && host.status === "offline" && host.wakeable) {
      const waking = wakingHostId === host.host_id;
      return (
        <DropdownMenuItem
          key={host.host_id}
          onSelect={(event) => {
            // Keep the menu open so the progress subtitle stays visible while
            // the box boots (~1 min).
            event.preventDefault();
            if (waking) return;
            setWakeError(null);
          ...
          title={`${host.name} — offline, can be started`}
        >
          <HostOption
            host={host}
