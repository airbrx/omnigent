...
function renderLanding(
  ...
  // airbrx: almost every caller wants the bare screen; the AgentDrawer
  // hand-off tests below swap this for a screen-plus-trigger harness so they
  // can navigate ("/" -> "/?agent=…") without unmounting it — the exact
  // same-route condition the fix targets. Deliberately fourth so upstream's
  // own (infoOverrides, route, onRender) call sites stay untouched at sync.
  element?: ReactNode,
) {
...
describe("AgentDrawer hand-off (?agent= param)", () => {
  ...
  it("updates the picker when an agent is picked while already on the landing screen", () => {
    ...
    renderLanding({}, "/", undefined, <Harness />);

    // No stored preference yet, so the picker starts on the catalog default
    // (a1, "Claude Code"). Read the agent off the trigger's accessible name,
    // not its text: since v0.14 the trigger renders the *model* and carries
    // the agent name in the accessible name only (upstream asserts the same
    // way, and explicitly asserts the name is absent from the text content).
    expect(screen.getByTestId("new-chat-landing-agent-select")).toHaveAccessibleName(
  ...
  it("wins over a parked landing draft and the persisted last-agent preference", () => {
    ...
    expect(screen.getByTestId("new-chat-landing-agent-select")).not.toHaveAccessibleName(
      expect.stringContaining("Codex"),
    );
  });
});
