...
/**
 * Rename a project by re-labeling every session filed under it from `from` to
 * `to`. A project is just the `omni_project` label on its sessions, so there's
 * no project row to rename — we move each member. Mirrors useDeleteProject's
 * all-or-report semantics: sessions the caller can't modify surface as
 * failures while the rest are moved. Renaming onto an existing project name
 * merges the two. Throws `{ failed, succeeded, total }` if any session
 * couldn't be relabeled, leaving those in place.
 */
