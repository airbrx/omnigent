...
def callback_client(
    ...
    Indirect parametrization (``request.param``, default ``False``): a
    bool sets the config's ``skip_email_verification`` flag; a dict is
    passed to :func:`_oidc_config` as keyword arguments instead (the
    admin-claim and email-claim tests use this).
    """
    keys = _IdpKeys()
...
def test_oidc_accepts_es384_id_token(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # Fork keeps the tuple-returning ``_resolve_oidc_identity`` (email, claims)
    # so the admin-claim can read the group claim; email is element [0].
    assert _resolve_oidc_identity({"id_token": token}, _oidc_config())[0] == "Alice@Example.com"
