...
def test_upsert_persists_version(host_store: HostStore) -> None:
    ...
    """
    host_store.upsert_on_connect(
        host_id="0000000000000000000000000000a001",
        name="laptop",
        user_id="alice@example.com",
        version="0.3.1",
    ...
    fetched = host_store.get_host("0000000000000000000000000000a001")
    assert fetched is not None
...
def test_upsert_reconnect_keeps_last_known_version_when_unreported(
    ...
    host_store.upsert_on_connect(
        host_id="0000000000000000000000000000a002",
        name="laptop2",
        user_id="alice@example.com",
        version="0.3.0",
    )
    ...
    assert host_store.get_host("0000000000000000000000000000a002").version == "0.3.2"  # type: ignore[union-attr]
...
def test_upsert_defaults_visibility_none_private(host_store: HostStore) -> None:
    ...
    """
    host = host_store.upsert_on_connect(
        host_id="0000000000000000000000000000a000", name="laptop", user_id="alice@x"
    )
    assert host.visibility is None
    assert host_store.get_host("0000000000000000000000000000a000").visibility is None
...
def test_upsert_records_host_declared_sharing(host_store: HostStore) -> None:
    ...
    got = host_store.get_host("0000000000000000000000000000a001")
    assert got.visibility == "shared" and got.workroot == "/srv/work"

    # Reconnect without --shared -> back to private (NULL), workroot cleared.
    host_store.upsert_on_connect(
        host_id="0000000000000000000000000000a001", name="box", user_id="alice@x"
    )
    back = host_store.get_host("0000000000000000000000000000a001")
...
def test_list_visible_hosts_unions_own_and_shared(host_store: HostStore) -> None:
    ...
    """
    host_store.upsert_on_connect(
        host_id="0000000000000000000000000000a1c0", name="a-laptop", user_id="alice@x"
    )
    host_store.upsert_on_connect(
        host_id="0000000000000000000000000000a1c1",
        name="a-box",
        user_id="alice@x",
        visibility="shared",
        workroot="/w",
    ...
    host_store.upsert_on_connect(
        host_id="0000000000000000000000000000b0b0", name="b-laptop", user_id="bob@x"
    )

    bob_visible = {h.host_id for h in host_store.list_visible_hosts("bob@x")}
    assert bob_visible == {"0000000000000000000000000000b0b0", "0000000000000000000000000000a1c1"}
    assert "0000000000000000000000000000a1c0" not in bob_visible

    # list_hosts (per-owner, used by the admin view) stays strict.
    assert {h.host_id for h in host_store.list_hosts("bob@x")} == {
        "0000000000000000000000000000b0b0"
    }
...
def test_list_visible_hosts_no_duplicate_for_owner_of_shared(host_store: HostStore) -> None:
    ...
    host_store.upsert_on_connect(
        host_id="0000000000000000000000000000d0d0",
        name="mine",
        user_id="alice@x",
        visibility="shared",
        workroot="/w",
    ...
    assert ids.count("0000000000000000000000000000d0d0") == 1
