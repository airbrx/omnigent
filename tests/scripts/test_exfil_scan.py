...
def test_keychain_accessor_name_not_blocked(tmp_path: Path) -> None:
    """A ``resolve_secret`` call + an httpx annotation does NOT block.
    ...
    also mentioned httpx — a type annotation was enough — tripped the
...
def test_bare_secret_identifier_still_blocks(tmp_path: Path) -> None:
    """The accessor carve-out does not cover a bare secret-named value.

    Guards the narrowing above from widening: ``client_secret`` read as a value
    and posted to the network is still the shape this scan exists to catch.
    ...
    proc = _run(
        ...
        _diff(
            "x.py",
            ['v = cfg["client_secret"]', "httpx.post(url, json=v)"],
        ),
    )
    assert proc.returncode != 0, proc.stdout
